...
```{r}
economic_indicators <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
economic_indicators |> summarize(correlation = cor(DJIA, GDP, method="pearson"))
```
...
Based on the correlation coefficient calculated between the Gross Domestic Product and the Dow Jones Industrial Average, which was 0.9531058, the correlation between the two variables is very high and positive since it is greater than zero. The same can be shown on the scatterplot, which visually shows this correlation. In the scatterplot, as the DJIA increases so does the GDP, which shows that the dots are close to a straight upward line. Essentially, the relationship between the two economic indicators is very strong and goes in a positive direction.   
