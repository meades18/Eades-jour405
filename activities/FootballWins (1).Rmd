...
This model can be useful in statistical analysis since it overviews the residuals, coefficients, r-squared and p-value, which are all essential in linear regression analysis. However, I think since the model has so many moving parts that it can be a bit difficult to focus on one aspect for analysis. 
...
The factor that has the most impact on wins is points scored, with the largest circle in the graphic. The factor that has the least influence on the wins is takeaways and points scored combined(?). 

## Using the Model

Without using the words "multiple regression model", describe how you could use this model to cover a team or game.

If I were to use this model to cover a team or game, I would describe this by saying that when yards gained and points scored are combined there is a higher chance of a team winning. 
